...
tools-docker-env-unset() {
  unset $(env | awk -F= '/DOCKER_/ {print $1}')
}

alias tools-docker-cleanup-images='docker images -q --filter "dangling=true" | xargs -P8 docker rmi'
alias tools-docker-cleanup-containers='docker rm $(docker ps -a -q)'
