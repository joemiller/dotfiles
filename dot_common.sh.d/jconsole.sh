...
#
#   $ tools-jcon target_host
#
# Connect to remote JVM on alternate port:
#
#   $ tools-jcon target_host 8181
#
...
tools-jcon() {
    local host=$1
    local jmx_port=${2:-7199}
    local local_proxy_port=$(find_available_port)
    ...
    local ssh_pid=$(ps ax | grep "jconsole-proxy-${local_proxy_port}" | grep -v grep | awk '{print $1}')
